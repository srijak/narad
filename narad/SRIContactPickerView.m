...
@interface SRIContactPickerView (){
...
@property (nonatomic, strong) SRIBubbleColor *bubbleColor;
@property (nonatomic, strong) SRIBubbleColor *bubbleSelectedColor;
...
- (void)setup {
  ...
  SRIContactBubble *contactBubble = [[SRIContactBubble alloc] initWithName:@"Sample"];
...
- (void)addContact:(id)contact withName:(NSString *)name {
  ...
  SRIContactBubble *contactBubble = [[SRIContactBubble alloc] initWithName:name
...
- (void)removeAllContacts
{
  for(id contact in [self.contacts allKeys]){
    SRIContactBubble *contactBubble = [self.contacts objectForKey:contact];
...
- (void)removeContact:(id)contact {
  ...
  SRIContactBubble *contactBubble = [self.contacts objectForKey:contactKey];
  [contactBubble removeFromSuperview];
...
- (void)setBubbleColor:(SRIBubbleColor *)color selectedColor:(SRIBubbleColor *)selectedColor {
  ...
  for (id contactKey in self.contactKeys){
    SRIContactBubble *contactBubble = (SRIContactBubble *)[self.contacts objectForKey:contactKey];
...
- (void)removeContactBubble:(SRIContactBubble *)contactBubble {
...
- (id)contactForContactBubble:(SRIContactBubble *)contactBubble {
...
- (void)layoutView {
  ...
  CGFloat maxFrameHeight = (lineCount + 1) * self.lineHeight + 2 * self.viewPadding; // grow view as required.
...
#pragma mark - SRIContactBubbleDelegate Functions
